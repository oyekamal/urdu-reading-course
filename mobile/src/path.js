...
function buildScreens(l, u, ctx) {
  ...
  switch (l.kind) {
    case 'letter': { const L = C.by[l.ch]; const pool = learned(); const sibs = (L.confusable || []).filter(c => pool.includes(c) && C.by[c]);
      ...
      const screens = [
      // 1 sound intro
      (box, cont) => { box.innerHTML = ''; const c = card(`<div class="ur big" style="font-size:130px">${L.ch}</div><h2 class="center">${L.name}</h2><p class="center muted">/${L.ipa}/ · ${L.hint}</p>`); const r = el('div', 'row'); r.style.justifyContent = 'center'; const b1 = el('button', 'btn btn-primary', '🔊 Say it again'); b1.onclick = () => play('names/' + L.id); const b2 = el('button', 'btn', `🔊 ${L.example[1]} · ${L.example[2]}`); b2.onclick = () => play('words/' + L.id); r.append(b1, b2); c.append(r, el('div', 'ur center', hl(W(L.example)))); box.append(c, cont('I heard it')); ctx.say('listen'); ctx.later(() => play('names/' + L.id), profile.track === 'child' ? 1500 : 200); },
      // 2 tap the sound among look-alikes
      (box, cont) => { box.innerHTML = ''; ctx.say('tap_heard'); box.append(el('p', 'muted', sibs.length ? `Listen, then tap. ${L.name} looks like ${sibs.map(c => C.by[c].name).join(', ')}: count the dots.` : 'Listen, then tap the letter you hear.')); box.append(D.tellApart(u, dctx, () => box.append(cont()), { letters: [L.ch], learned: pool, rounds: 5 })); if (pool.length < 2) box.append(cont()); },
